appcan.define("WelcomePageSlider", function ($, exports, module) {
    var model_item = '\
        <div class="slider-item ub-fh ub-fv ub-img1" id="<%=data.id%>" style="background-image:url(<%=data.img%>)">\
            <span class="uabs"><%=data.label%></span>\
        </div>';
    var model_Conitem = '\
        <div class="slider-item1 ub-fh ub-fv" id="<%=option.index%>">\
    ...
    SliderView.prototype = {
        ...
        set: function (data) {
            ...
            if (self.option.hasCircle) {
                self.Circle = $('<div class="label1" style="background-color:transparent;height:1.5em;width: 100%;position: absolute;left: 0px;bottom: 0px;"></div>');
                self.circle = self.circle || $('<div class="ulev-1 ut-s label sc-text-hint uinn-l1" style="background-color: transparent;"></div>');
            ...
            self.ele.off("swipeLeft").on("swipeLeft", function (evt) {
                ...
                self._moveTo(++self.option.index);
                self.autoMove(self.option.auto);
                self.emit("swipeLeft", self, index + 1, data[index + 1]);
            });
            self.ele.off("swipeRight").on("swipeRight", function (evt) {
                ...
                self._moveTo(--self.option.index);
                self.autoMove(self.option.auto);
                self.emit("swipeRight", self, index, data[index]);
            });
            self.option.hasSwipeLeftIntoCb = false;
            self.ele.off("swipeMoveLeft").on("swipeMoveLeft", function (evt) {
                var index = self.option.index + 1;
                if (index == self.option.itemCount) {
                    if (!self.option.swipeLeftInto) {
                        return false;
                    }

                    if (!self.option.hasSwipeLeftIntoCb && typeof self.option.swipeLeftIntoCb == "function") {
                        self.option.swipeLeftIntoCb();
                        self.option.hasSwipeLeftIntoCb = true;
                    }
                    var w = (-(self.option.index + 1.5) * self.ele.offset().width);
                    self.container.css("-webkit-transition", "all 550ms")
                            .css("opacity", "0")
                            .css("-webkit-transform", "translateX(" + w + "px)");
                }
            });
            self.ele.off("swipeMoveRight").on("swipeMoveRight", function (evt) {
                var index = self.option.index - 1;
        ...
        moveToLast: function () {
            var self = this;
            var w = (-(self.option.itemCount + 1) * self.ele.offset().width);
            self.container.css("-webkit-transform", "translateX(" + w + "px)");
        }
    };
