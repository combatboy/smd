		...
		getJSON : function(a, b,c,d,e,f,g,key,cert) {
			...
			if(window.location.href.substring(0,7).toLowerCase()=="file://")
			{
				if(a.substring(0,4).toLowerCase()=="http")
				{
					...
					if(cert)
						uexXmlHttpMgr.setCertificate(zy_json_opid,'',cert);
					if(f)
					{
						for(var it in f)
						{
					...
					uexWidgetOne.cbError= function(inOpId,inErrorCode,inErrorDes){
			       {
					...
					uexFileMgr.cbIsFileExistByPath = function(opId,dataType,data){
					    ...
					    if(data==1){
								...
								str = JSON.parse(str);
								if(!(zy_json_session[opId].errcb===undefined))
									zy_json_session[opId].errcb(str);
								delete zy_json_session[opId];
								return;
							}
					  }
					}
