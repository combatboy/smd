...
public class AppDownTask extends MyAsyncTask {
	...
	public AppDownTask(AppBean appBean, ExpandableListView listview,
			int inType, int position) {
		if (appBean == null) {
	...
	protected Object doInBackground(Object... params) {
		...
		try {
			...
			if (fileSize > 0 && fileSize == count) {
				Log.e(EUExAppMarketEx.TAG, "" + tmpFile.getAbsolutePath());
				if (tmpFile.exists()) {
			...
			String netExtra = CommonUtility.getNetExtras(listview.getContext());
			...
			if (conn.getResponseCode() == HttpURLConnection.HTTP_OK
				...
				if (is != null) {
					...
					while (!isCancelled()
						...
						CommonUtility.saveProgress(mContext, appBean,
								(int) downloadPercent);
	...
	public void handleOnPreLoad(MyAsyncTask task) {
		if (type == 0) {
			...
			if (appBean != null) {
				int progress = CommonUtility.getProgress(mContext, appBean);
				mProgress = progress;
	...
	public void handleOnUpdateProgress(MyAsyncTask task, int percent) {
		if (type == 0) {
			...
			BDebug.i(TAG, appBean.getAppName() + "-->update:" + percent);
		}
	}
	...
	private void creatTaskTable() {
		...
		mDatabaseHelper = new DatabaseHelper(listview.getContext(),
				"Downloader.db", 1);
	...
	private void showProgress(AppBean appBean, int progress) {
		if (listview != null) {
			...
			String taskPos = mAdapter.getItemPosition(appBean);
